...
const plans = [
  {
    ...
    missing: ['Онлайн-оплата', 'Приоритетная поддержка', 'Кастомный домен'],
  },
  {
    name: 'Pro',
    price: '1 490',
...
const faq = [
  {
    q: 'Можно ли перейти с Free на Pro в любой момент?',
    a: 'Да. Upgrade происходит мгновенно, оплата берётся пропорционально оставшимся дням месяца.',
  },
  {
    q: 'Есть ли скидки при оплате за год?',
    a: 'Да — при оплате на год скидка 20%. Свяжитесь с нами для оформления.',
  },
  {
    q: 'Что происходит с данными при отмене подписки?',
    a: 'Данные хранятся 30 дней после отмены. Вы можете экспортировать их в любой момент.',
  },
  {
    q: 'Есть ли пробный период для платных тарифов?',
    a: 'Да — 14 дней бесплатно на тарифе Pro. Карта не требуется.',
  },
]
          ...
          {plans.map(plan => (
            <div key={plan.name}
              ...
              <ul className="space-y-2.5 flex-1">
                {plan.features.map(f => (
                  <li key={f} className={`flex items-center gap-2 text-sm ${plan.color.includes('blue-600') ? 'text-white' : 'text-gray-600'}`}>
