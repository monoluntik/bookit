...
import { formatDate, formatTime, BUSINESS_TYPE_META } from '@/lib/businessTypes'
...
const STATUS_LABELS: Record<string, string> = {
  ...
  CANCELLED: 'Отменена', NO_SHOW: 'Не пришёл', BLOCK: 'Блокировка',
}

function DonutChart({ data }: { data: Record<string, number> }) {
  const entries = Object.entries(data).filter(([, v]) => v > 0)
  const total = entries.reduce((s, [, v]) => s + v, 0)
  if (!total) return <p className="text-sm text-gray-400 py-4 text-center">Нет данных</p>
...
export default function StatsPage() {
  ...
  stats.upcomingBookings?.forEach((b: any) => {
    const name = b.service?.name ?? 'Без услуги'
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {[
          {
            label: 'Брони сегодня', value: stats.today,
            sub: `${stats.month} за месяц`,
            color: 'text-blue-600', bg: 'bg-blue-50',
          },
          {
            label: 'Выручка за месяц',
            value: `${Number(stats.revenueMonth).toLocaleString('ru')} с`,
            sub: revenueΔ ? `${revenueΔ.up ? '↑' : '↓'} ${Math.abs(revenueΔ.d)}% к прошлому` : 'нет данных',
            color: 'text-green-600', bg: 'bg-green-50',
            subColor: revenueΔ?.up ? 'text-green-500' : 'text-red-500',
          },
          {
            label: 'Брони за месяц', value: stats.month,
            sub: bookingsΔ ? `${bookingsΔ.up ? '↑' : '↓'} ${Math.abs(bookingsΔ.d)}% к прошлому` : 'нет данных',
            color: 'text-indigo-600', bg: 'bg-indigo-50',
            subColor: bookingsΔ?.up ? 'text-green-500' : 'text-red-500',
          },
          {
            label: 'Ожидают подтверждения', value: stats.pending,
            sub: `Всего броней: ${stats.total}`,
      ...
      <div className="grid lg:grid-cols-2 gap-4 mb-4">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          <div className="text-sm font-semibold text-gray-700 mb-3">Брони за последние 30 дней</div>
        ...
        {/* Status breakdown */}
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          <div className="text-sm font-semibold text-gray-700 mb-3">Статусы броней</div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        {/* Upcoming bookings */}
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          <div className="text-sm font-semibold text-gray-700 mb-3">Ближайшие записи</div>
          {stats.upcomingBookings?.length === 0 ? (
            <p className="text-sm text-gray-400">Предстоящих записей нет</p>
          ) : (
        ...
        {/* Service popularity */}
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          <div className="text-sm font-semibold text-gray-700 mb-3">Популярность услуг</div>
          {serviceEntries.length === 0 ? (
            <p className="text-sm text-gray-400">Нет данных по предстоящим записям</p>
          ) : (
