...
export default function SchedulePage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      success(formClosed ? 'Выходной добавлен' : 'Особые часы сохранены')
      // Upsert locally
      setExceptions(prev => {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-gray-900">Выходные и отпуска</h1>
      ...
      <div className="bg-white rounded-2xl p-4 mb-5 flex flex-wrap gap-3 shadow-sm">
        ...
        {resources.length === 0 && (
          <span className="text-sm text-gray-400">Нет ресурсов в этом бизнесе</span>
        )}
      </div>
      ...
      {adding && (
        <form onSubmit={handleAdd} className="bg-white rounded-2xl p-5 mb-5 shadow-sm">
          <h3 className="font-semibold text-gray-800 mb-4">Новое исключение</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {!formClosed && (
              <>
                <div />
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-1">Начало работы</label>
                  <input type="time" value={formStart} onChange={e => setFormStart(e.target.value)}
                    className="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" />
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-1">Конец работы</label>
            ...
            <div className={formClosed ? '' : 'sm:col-span-2'}>
              <label className="text-xs font-medium text-gray-600 block mb-1">Причина (необязательно)</label>
              <input type="text" value={formReason} onChange={e => setFormReason(e.target.value)}
                placeholder={formClosed ? 'Отпуск, болезнь, праздник...' : 'Сокращённый день...'}
                className="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" />
            </div>
          </div>
          ...
          <p className="text-xs text-gray-400 mt-3 mb-4">
            {formClosed
              ? 'Клиенты не смогут бронировать этот день. Существующие брони не затрагиваются.'
              : 'Клиенты смогут бронировать только в указанное время вместо обычного расписания.'}
          </p>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-12 text-center text-gray-400">
          ...
          <div>Выберите ресурс чтобы управлять выходными</div>
        </div>
      ) : (
        <div className="space-y-3">
          {upcoming.length === 0 && past.length === 0 && (
            <div className="bg-white rounded-2xl p-12 text-center">
              ...
              <p className="text-gray-400 mb-4">Нет запланированных выходных и исключений</p>
          ...
          {upcoming.length > 0 && (
            <>
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide px-1">Предстоящие</h2>
              {upcoming.map(ex => (
          ...
          {past.length > 0 && (
            <>
              <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wide px-1 mt-4">Прошедшие</h2>
              {past.map(ex => (
                <ExceptionRow key={ex.id} ex={ex} onDelete={handleDelete} isPast />
              ))}
            </>
          )}
        </div>
...
function ExceptionRow({ ex, onDelete, isPast = false }: {
    ...
    <div className={`bg-white rounded-2xl px-5 py-4 shadow-sm flex items-center gap-4 ${isPast ? 'opacity-50' : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-sm text-gray-500 mt-0.5">
          {ex.isClosed
            ? 'Выходной / закрыто'
            : `Сокращённый день: ${ex.startTime}–${ex.endTime}`}
