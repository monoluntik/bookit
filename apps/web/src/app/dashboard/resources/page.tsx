...
export default function ResourcesPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!token) return
    if (scheduleForm.days.length === 0) { setFormError('Выберите хотя бы один день'); return }
    if (scheduleForm.start >= scheduleForm.end) { setFormError('Время начала должно быть раньше конца'); return }
    setSaving(true)
    setFormError('')
    try {
  ...
  const handleSaveSchedule = async (e: React.FormEvent) => {
    ...
    if (editScheduleForm.days.length === 0) { setFormError('Выберите хотя бы один день'); return }
    if (editScheduleForm.start >= editScheduleForm.end) { setFormError('Время начала должно быть раньше конца'); return }
    setSaving(true)
    setFormError('')
    try {
  ...
  const ScheduleFields = ({
    ...
    <div className="pt-3 border-t border-gray-100">
      <div className="text-xs font-medium text-gray-500 mb-3">Расписание</div>

      {/* Days */}
      <div className="mb-3">
        <div className="text-xs text-gray-400 mb-1.5">Рабочие дни</div>
      ...
      {/* Times */}
      <div className="mb-3">
        <div className="text-xs text-gray-400 mb-1.5">Рабочие часы</div>
      ...
      {/* Slot duration */}
      <div>
        <div className="text-xs text-gray-400 mb-1.5">Длина слота</div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-gray-900">Ресурсы</h1>
      ...
      <div className="space-y-3">
        {resources.length === 0 ? (
          <div className="bg-white rounded-2xl p-10 text-center text-gray-400">
            ...
            <p className="mb-1">Ресурсов ещё нет</p>
            <p className="text-xs">Нажмите «+ Добавить ресурс» чтобы создать мастера, стол или комнату</p>
          </div>
        ) : resources.map(r => {
            ...
            <div key={r.id} className="bg-white rounded-2xl p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  {r.capacity && <div className="text-xs text-gray-400 mt-0.5">Вместимость: {r.capacity}</div>}
                  ...
                  {sched ? (
                    <div className="mt-2 space-y-1">
                      ...
                      <div className="text-xs text-gray-500">
                        ...
                        <span className="ml-2 text-gray-400">слоты по {sched.slotDurationMinutes} мин</span>
                      </div>
                    </div>
                  ) : (
                    <div className="mt-2 flex items-center gap-1.5 text-xs text-amber-600 bg-amber-50 rounded-lg px-2 py-1.5 w-fit">
                      <span>⚠️</span> Расписание не задано — клиенты не увидят слоты
                    </div>
                  )}
                </div>
      ...
      {showNew && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl max-h-[90vh] overflow-y-auto"
            ...
            <h2 className="font-semibold text-gray-900 mb-4">Новый ресурс</h2>
            <form onSubmit={handleCreate} className="space-y-3">
              <input required placeholder="Название (мастер, стол, номер...)" value={form.name}
              ...
              <input placeholder="Описание (необязательно)" value={form.description}
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" />
              <div>
                <label className="text-xs text-gray-400 mb-1 block">Вместимость (человек)</label>
      ...
      {editScheduleFor && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl max-h-[90vh] overflow-y-auto"
            role="dialog" aria-modal="true">
            <h2 className="font-semibold text-gray-900 mb-1">Расписание</h2>
