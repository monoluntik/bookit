        ...
        {staff.length === 0 ? (
          ...
          <div key={s.id} className="bg-white rounded-2xl p-4 shadow-sm flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              {s.role && <div className="text-xs text-purple-600 mt-0.5">{t('roleLabel', { role: s.role.name })}</div>}
            </div>
            ...
            <form onSubmit={handleAdd} className="space-y-3">
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setShowAdd(false)}
                  className="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-600">{t('cancel')}</button>
