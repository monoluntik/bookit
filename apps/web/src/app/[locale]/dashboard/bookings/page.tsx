...
function CalendarView({ bookings, onDateClick, statusLabels, dayLabels, blockLabel }: {
  bookings: any[]; onDateClick: (d: string) => void
  statusLabels: Record<string, string>; dayLabels: string[]; blockLabel: string
}) {
      ...
      <div className="flex flex-wrap gap-3 px-5 py-3 border-t border-gray-100">
        {STATUSES.map(s => (
          <div key={s} className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-gray-400">{statusLabels[s]}</span>
          </div>
        ))}
...
function OwnerBookingModal({ businesses, token, onClose, onCreated }: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      success(mode === 'BLOCK' ? t('successBlock') : t('successManual'))
      onCreated(data)
      onClose()
    } catch (err: any) {
      ...
      <div className="bg-white w-full sm:rounded-2xl sm:max-w-md shadow-xl max-h-[95vh] overflow-y-auto"
        ...
        <div className="px-5 pt-4">
          ...
          <p className="text-xs text-gray-400 mt-2 mb-4">
            {mode === 'MANUAL' ? t('manualHint') : t('blockHint')}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-5 pb-5 space-y-4">
          ...
          {businesses.length > 1 && (
            <div>
              <label className="text-xs font-medium text-gray-600 block mb-1">{t('business')}</label>
          ...
          {/* Resource */}
          <div>
            <label className="text-xs font-medium text-gray-600 block mb-1">{t('resource')}</label>
            {loadingRes ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-gray-600 block mb-1">{t('startTime')}</label>
            ...
            <div>
              <label className="text-xs font-medium text-gray-600 block mb-1">{t('endTime')}</label>
          ...
          {mode === 'MANUAL' && (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-1">{t('guestName')}</label>
                  <input type="text" value={form.guestName} onChange={set('guestName')}
                    placeholder={t('guestNamePlaceholder')}
                    className="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" />
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-1">{t('guestPhone')}</label>
              ...
              {services.filter(s => !form.resourceId || s.resourceId === form.resourceId || !s.resourceId).length > 0 && (
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-1">{t('service')}</label>
          ...
          <div>
            <label className="text-xs font-medium text-gray-600 block mb-1">
              {mode === 'BLOCK' ? t('blockReason') : t('note')}
            </label>
            <textarea value={form.notes} onChange={set('notes')} rows={2}
              placeholder={mode === 'BLOCK' ? t('blockReasonPlaceholder') : t('notePlaceholder')}
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button type="submit" disabled={saving || !!timeError}
              ...
              {saving ? t('saving') : mode === 'BLOCK' ? t('submitBlock') : t('submitManual')}
            </button>
          </div>
        </form>
      </div>
    ...
    <div>
      ...
      {loading ? (
        ...
        <CalendarView bookings={bookings} onDateClick={d => { setView('list'); setDate(d) }}
          statusLabels={STATUS_LABELS} dayLabels={dayLabels} blockLabel={t('blockLegend')} />
      ) : bookings.length === 0 ? (
