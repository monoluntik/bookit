...
export default function MyBookingsPage() {
  ...
  const handleCancel = async (id: string) => {
    if (!token || !confirm('Отменить бронь?')) return
    setCancelling(id)
    try {
      ...
      <header className="bg-white border-b border-gray-100">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <button onClick={() => { logout(); router.push('/') }} className="text-gray-400 hover:text-gray-700">Выйти</button>
          </div>
        </div>
      </header>

      <div className="max-w-2xl mx-auto px-4 py-6">
        <h1 className="text-xl font-bold text-gray-900 mb-6">Мои бронирования</h1>

        {bookings.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 text-center">
            <div className="text-4xl mb-3">📅</div>
            <p className="text-gray-500 mb-4">У вас пока нет броней</p>
          ...
          <>
            {upcoming.length > 0 && (
              <div className="mb-6">
                <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Предстоящие</h2>
            ...
            {past.length > 0 && (
              <div>
                <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Прошедшие</h2>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          ...
          {b.payment?.status === 'PAID' && (
            <div className="text-xs text-green-600 mt-1">✓ Оплачено</div>
          )}
        </div>
